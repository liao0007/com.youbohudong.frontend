...
export default class Ellipsis extends Component {
  ...
  render() {
    ...
    if (!lines) {
      return (
        <EllipsisText className={cls} length={length} text={children || ''}
                      tooltip={tooltip} {...restProps} />);
    }

    const id = `antd-pro-ellipsis-${`${new Date().getTime()}${Math.floor(Math.random() *
      100)}`}`;
      ...
      >
        ...
        >
          ...
          <div className={styles.shadow}
               ref={this.handleShadowChildren} >{children}</div >
